...
export async function checkVideoStatusAction(requestId: string) {
    ...
    try {
        ...
        // Try multiple locations for status field
        let status = jobData.status || data.status || jobData.state || data.state
        ...
        if (status === 'success' || status === 'completed') {
            ...
            // Try to extract URL from resultJson (which is a stringified JSON)
            if (jobData.resultJson) {
                try {
                    ...
                    // Kie.ai Sora 2 returns { resultUrls: ["url"] }
            ...
            // Fallback: If not found in resultJson, try direct fields
            if (!videoUrl) {
                videoUrl = jobData.result?.video_url || jobData.video_url || jobData.url || jobData.output?.url || jobData.data?.videoUrl
            }

            if (!videoUrl) {
                console.error('CRITICAL: Status is completed but could not find Video URL in response.', jobData)
            }

        } else if (status === 'fail' || status === 'failed') {
            newStatus = 'failed'
            console.error('Video Generation Failed:', jobData.failMsg || 'Unknown error')
        } else if (['waiting', 'queuing', 'generating', 'processing'].includes(status)) {
            // Update to specific status if it matches known intermediate states
            newStatus = status
        } else {
            // Fallback for unknown status, log FULL response
            console.log('Unknown Kie.ai status:', status)
            console.log('Full Kie.ai Response:', JSON.stringify(data, null, 2))
        }
